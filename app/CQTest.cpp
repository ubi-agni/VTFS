...
//                std::cout<<"local rotate2 "<<local_r_v.second<<std::endl;
                left_ac_vec[i]->update_controller_para(local_r_v,left_task_vec[i]->curtaskname.prot);
            }
            global2local(activel_vel,kuka_left_arm->get_cur_cart_o(),local_l_v);
            left_ac_vec[i]->update_controller_para(local_l_v,left_task_vec[i]->curtaskname.prot);
            mutex_vel.unlock();
