...
//data record for learning the mapping from force to desired eef.
std::ofstream TInitOrien,Tpose, ToolOriginposition;

Eigen::Matrix3d InitOrien;

bool record_f_flag;
bool toolposition_record;
...
void dmptest_cb (boost::shared_ptr<std::string> data){
	toolposition_record = true;
	des_vec(0) = 0;
    des_vec(1) = 1.0;
    des_vec(2) = 0;
    mutex_act.lock();
    ...
    std::string dmpFileName = "/homes/qli/SWLib/dmp_kit/dmp_test/eightfig.csv";
    ProActController* dmpC = new ProActController(*right_pm, dmpFileName);
    DMP::DVec goal;
    goal.push_back(0);
    goal.push_back(0);
    goal.push_back(0);
    ...
    std::vector<DMP::DMPState > initState;
    //Eigen::Vector3d robotPosition = right_rs->robot_position["eef"];
    //for(size_t i = 0; i < 3; ++i)
    {
        DMP::DMPState cState;
        cState.pos = 0;
        cState.vel = 0;
        initState.push_back(cState);
    }    
    ...
    dmpC->dmpCtrl->prepareExecution(goal, initState, 1.0, 1.0);
    dmpC->currentState = initState;    
    dmpC->canVal = timeDuration;
    dmpC->motionTimeDuration = timeDuration;
    dmpC->init_position.setZero();
    dmpC->init_position<<right_newP_x,right_newP_y,right_newP_z;

    dmpC->origForce = cf_tm.col(0);
    dmpC->contactFrame = cf_tm;

    right_ac_vec.push_back(dmpC);
    ...
    right_task_vec.back()->Ssrc = DMP_FEEDBACK;
...
void record_init_orien_cb(boost::shared_ptr<std::string> data){
	InitOrien = right_rs->robot_orien["eef"];
    std::cout<<"recording init orientation data"<<std::endl;
}
...
void init(){
    ...
    ToolOriginposition.open((data_f+std::string("toolposition.txt")).c_str());
    ...
    boost::function<void(boost::shared_ptr<std::string>)> button_dmptest(dmptest_cb);
    boost::function<void(boost::shared_ptr<std::string>)> button_slidingcylinder(slidingcylinder_cb);
    boost::function<void(boost::shared_ptr<std::string>)> button_record_init_orien(record_init_orien_cb);
    boost::function<void(boost::shared_ptr<std::string>)> button_record_training(record_training_cb);
    ...
    com_rsb->register_external("/foo/dmptest",button_dmptest);
    com_rsb->register_external("/foo/slidingcylinder",button_slidingcylinder);
    com_rsb->register_external("/foo/record_init",button_record_init_orien);
    com_rsb->register_external("/foo/record_training",button_record_training);
