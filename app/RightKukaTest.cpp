...
/*
 ...
 Description : right kuka tactile servo test
 ============================================================================
 */
...
void contact_cb(void){
    ...
    ac->set_init_TM(kuka_right_arm->get_cur_cart_o());
...
//        kuka_right_arm->update_robot_stiffness();
        ...
        mutex_act.lock();
        if(task->mt == JOINTS)
            ac->update_robot_reference(kuka_right_arm,task);
        if(task->mt == TACTILE){
            mutex_tactile.lock();
            ac->update_robot_reference(kuka_right_arm,task,&right_myrmex_msg);
...
void tactileviarsb(){
    ...
    std::cout<<"contact information "<<right_myrmex_msg.cf<<","<<right_myrmex_msg.cogx<<","<<right_myrmex_msg.cogy<<","<<right_myrmex_msg.contactflag<<std::endl;
    mutex_tactile.unlock();
}
...
void init(){
    ...
    com_okc->connect();
    kuka_right_arm = new KukaLwr(kuka_right,*com_okc);
    ac = new ProActController(*pm);
    task = new KukaSelfCtrlTask(RP_NOCONTROL);
    Eigen::Vector3d p,o;
    p.setZero();
    ...
    task->set_desired_p_eigen(p);
    task->set_desired_o_ax(o);
    kuka_right_arm->setAxisStiffnessDamping(ac->pm.stiff_ctrlpara.axis_stiffness, \
